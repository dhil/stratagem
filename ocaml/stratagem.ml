...
module Forest(Nat : NAT_UTILS with type nat = nat): sig
  ...
  type forest = Forest of (nat -> (nat * forest))
  let forest : (nat -> nat * forest) -> forest
    = fun f -> Forest f
  ...
  let rec apply : forest -> forest -> forest
    = fun f g ->
    let rec apply'' : (nat * forest) -> forest -> (nat * forest)
      = fun (n, f) g ->
      if is_ans n then
        let (j, g') = g % n in
        apply'' (f %% j) g'
  ...
  type _ Effect.t += Branch : nat -> (nat * forest) Effect.t
  type branch = nat -> nat * forest

  exception Non_linear_error
  let error_branch : branch
    = fun _ -> raise Non_linear_error


  let lambda : (forest -> forest) -> forest
    = fun phi ->
    let rec lambda' : (branch -> forest) -> forest
      = fun p ->
      forest
        (fun h ->
          lambda''
            (fun g ->
              let f = p g in
              f % h))
    and lambda'' : (branch -> nat * forest) -> nat * forest
      = fun r ->
      Effect.Deep.match_with r (fun q -> Effect.perform (Branch q)) hbranch
    and hbranch =
      let open Effect.Deep in
      { retc = (fun (ans, p') -> (ans, lambda' (fun _ -> p')))
      ; exnc = raise
           ...
           Some (fun (k : (a, _) continuation) ->
               (q, forest
                     (fun (a : nat) ->
                       lambda''
                         (fun (br : branch) ->
                           continue k (a, forest br)))))
        | _ -> None) }
    in
    lambda' (fun f -> phi (forest f))
